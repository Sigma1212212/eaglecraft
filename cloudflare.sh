...
if [ "$MODE" = "named" ]; then
  ...
  case "$(uname -s)" in
    MINGW*|MSYS*|CYGWIN*)
      CRED_OUT="$(echo "$CRED" | sed 's|^/\([a-z]\)/|\U\1:/|')"
      CERT_OUT="$(echo "$CERT" | sed 's|^/\([a-z]\)/|\U\1:/|')" ;;
    *)
      CRED_OUT="$CRED"
      CERT_OUT="$CERT" ;;
  esac
  ...
  # overrides everything below, so the second hostname would 404.
  {
    ...
    # UUID, not name: resolving a name needs cert.pem, which a LocalSystem
    # service cannot find in this user's profile.
    echo "tunnel: $TUNNEL_ID"
    echo "origincert: $CERT_OUT"
